...
{
    fn create_session<'a>(&'a self, id: &'a uuid::Uuid, role: Box<dyn ToString + Send + 'a>, refresh_key_lifetime_days: i64, ip_addr: &'a str, fingerprint: &'a str, audience: Option<&[Box<dyn ToString + Sync>]>) -> Pin<Box<dyn Future<Output = Result<uuid::Uuid, Error>> + Send + 'a>>;
    fn get_session<'a>(&'a self, session_id: &'a uuid::Uuid) -> Pin<Box<dyn Future<Output = Result<UserSessionDbo, Error>> + Send + 'a>>;
    fn insert_or_replace_session<'a>(&'a self, session: UserSessionDbo) -> Pin<Box<dyn Future<Output = Result<(), Error>> + Send + 'a>>;
    fn sessions_count<'a>(&'a self, id: &'a uuid::Uuid) -> Pin<Box<dyn Future<Output = Result<u32, Error>> + Send + 'a>>;
    fn delete_all_sessions<'a>(&'a self, id: &'a uuid::Uuid) -> Pin<Box<dyn Future<Output = Result<u64, Error>> + Send + 'a>>;
    fn delete_session<'a>(&'a self, session_id: &'a uuid::Uuid) -> Pin<Box<dyn Future<Output = Result<(), Error>> + Send + 'a>>;
    fn update_session_key<'a>(&'a self, session_id: &'a uuid::Uuid, refresh_key_lifetime_days: i64) -> Pin<Box<dyn Future<Output = Result<(), Error>> + Send + 'a>>;
}
...
impl IAuthorizationRepository for AuthorizationRepository
{
    fn create_session<'a>(&'a self, id: &'a uuid::Uuid, role:  Box<dyn ToString + Send + 'a>, refresh_key_lifetime_days: i64, ip_addr: &'a str, fingerprint: &'a str, audience: Option<&[ Box<dyn ToString + Sync>]>) -> Pin<Box<dyn Future<Output = Result<uuid::Uuid, Error>> + Send + 'a>>
    {
        let audience: Vec<String> = audience.unwrap_or(&Vec::new()).iter().map(|a| a.to_string()).collect();
        Box::pin(async move 
        {
...
fn new_session<'a>(id: &uuid::Uuid, role: Box<dyn ToString + Send + 'a>, refresh_key_lifetime_days: i64, ip_addr: &str, fingerprint: &str, audience: Vec<String>) -> UserSessionDbo
{
    UserSessionDbo
    {
        id: id.clone(),
